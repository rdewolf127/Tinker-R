...
# create a function to build the map layers
map_function <- function(var_name, legend_title) {
  ggp <- ggplot(data=geoforti, aes(x=long, y=lat, group=group))   #create ggplot object
  ggp <- ggp + geom_polygon(aes(fill=as.numeric(var_name)))         # draw polygons
  ggp <- ggp + geom_path(color="grey", linetype=2)  # draw boundaries
  ...
  ggp <- ggp + labs(title="New York", fill=legend_title)
...
map_function(geoforti$HC05_EST_VC05,"Moved From Abroad 18-24 Years")
map_function(geoforti$HC05_EST_VC06,"Moved From Abroad 25-34 Years")
map_function(geoforti$HC05_EST_VC07,"Moved From Abroad 35-44 Years")
map_function(geoforti$HC05_EST_VC08,"Moved From Abroad 45-54 Years")
